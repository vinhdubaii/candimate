...
function updateLb() {
  ...
  const el = $('lb-img');
  setZoomScale(1, { forceUI: true }); // reset zoom + pan về mặc định khi đổi ảnh
...
/* ── ZOOM + PAN + PINCH ──
   zoomScale là 1 giá trị LIÊN TỤC từ 1x (không zoom) đến ZOOM_MAX (3x),
   thay vì chỉ bật/tắt nhị phân — để pinch 2 ngón zoom mượt theo đúng độ mở tay. */
const ZOOM_MAX = 3, ZOOM_MIN = 1;
let zoomScale = 1;
let panX = 0, panY = 0;
...
function getMaxPan() {
  const img = $('lb-img');
  return {
    maxX: (img.offsetWidth  * (zoomScale - 1)) / 2,
    maxY: (img.offsetHeight * (zoomScale - 1)) / 2,
  };
}
...
function setZoomScale(scale, opts) {
  opts = opts || {};
  ...
  // Kẹp lại pan theo đúng mức zoom mới, tránh vượt giới hạn khi scale giảm
  const { maxX, maxY } = getMaxPan();
  panX = isZoomed ? Math.min(maxX, Math.max(-maxX, panX)) : 0;
  panY = isZoomed ? Math.min(maxY, Math.max(-maxY, panY)) : 0;
  applyZoomTransform();

  if (isZoomed !== wasZoomed || opts.forceUI) {
    img.classList.toggle('zoomed', isZoomed);
    $('lightbox')?.classList.toggle('zoomed-active', isZoomed);
    ...
    btn.childNodes[btn.childNodes.length - 1].textContent = isZoomed ? ' Thu nhỏ' : ' Phóng to';
  }
}

/* Tap nhẹ (không kéo) → nhảy nhanh 1x ⇄ 3x. Vẫn giữ song song với pinch. */
function toggleZoom() {
  setZoomScale(zoomScale > 1 ? 1 : ZOOM_MAX);
}

/* 1 ngón: pan/tap — chuột & cảm ứng dùng chung.
   2 ngón (cảm ứng): pinch zoom liên tục. */
;(function initLbPanZoom() {
  ...
  function onUp() {
    ...
    if (!panMoved) toggleZoom(); // nhấn nhẹ → toggle zoom (cả 2 chiều)
  }
  ...
  img.addEventListener('touchstart', e => {
    e.preventDefault(); // Ngăn trình duyệt tạo mouse event ảo → tránh double-toggle
    if (e.touches.length === 2) {
      isPanning  = false; // huỷ pan 1 ngón dở dang nếu có
      isPinching = true;
  ...
  img.addEventListener('touchmove', e => {
    ...
    if (!isPinching && e.touches.length === 1) {
      if (zoomScale > 1) e.preventDefault(); // ngăn cuộn trang khi đang pan
  ...
  img.addEventListener('touchend', e => {
    if (isPinching) {
      if (e.touches.length < 2) {
        isPinching = false;
        if (zoomScale < 1.08) setZoomScale(1); // buông tay gần 1x → snap về hẳn 1x
        if (e.touches.length === 1) onDown(e.touches[0].clientX, e.touches[0].clientY); // còn 1 ngón → chuẩn bị pan tiếp
      }
      return;
    }
    onUp();
